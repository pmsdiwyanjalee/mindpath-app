...
class _AdminUsersManagementScreenState extends State<AdminUsersManagementScreen>
  ...
  Future<void> _loadUsers() async {
    _users = await _adminService.getAllAppUsers();
  ...
  List<AppUser> get _activeUsers =>
      _filteredUsers.where((u) => u.status == 'active').toList();
  List<AppUser> get _suspendedUsers =>
      _filteredUsers.where((u) => u.status == 'suspended').toList();
  List<AppUser> get _allFilteredUsers => _filteredUsers;
              ...
              children: [
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                _allFilteredUsers.length.toString(),
                ...
                Expanded(
                  child: TabBarView(
                    controller: _tabController,
                    children: [
                      _buildUserList(_allFilteredUsers),
                      _buildUserList(_activeUsers),
                      _buildUserList(_suspendedUsers),
                    ],
                  ),
                ),
              ],
    ...
    if (users.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.people_outline_rounded, color: _textMuted, size: 48),
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: _infoChip(
                          'Days Sober', user.daysSober.toString(), _accentBlue),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _infoChip(
                          'Streak', user.streakDays.toString(), _successGreen),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _infoChip('Sessions',
                          user.totalSessions.toString(), _warningOrange),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
                ...
                // ── Metadata ────────────────────────────────────────────
                Text(
                  'Registered: ${user.registrationDate.toString().split(' ')[0]}',
                  style: const TextStyle(
                    color: _textMuted,
                    fontSize: 10,
                    fontWeight: FontWeight.w500,
        ...
        child: DraggableScrollableSheet(
          ...
          builder: (_, controller) => ListView(
            ...
            children: [
              ...
              _detailRow(
                  'Phone', 'Counselors: ${user.assignedCounselors.length}'),
              _detailRow('Days Sober', user.daysSober.toString()),
              _detailRow('Streak', user.streakDays.toString()),
              _detailRow('Total Sessions', user.totalSessions.toString()),
              _detailRow('Recovery Phase', user.recoveryPhase),
              _detailRow('Status', user.status),
              _detailRow(
                  'Registered', user.registrationDate.toString().split(' ')[0]),
              _detailRow('Last Login', user.lastLogin.toString().split(' ')[0]),
            ],
          ),
        ),
      ),
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text(
              user.status == 'active' ? 'Suspend' : 'Activate',
              style: TextStyle(
                color: user.status == 'active' ? _errorRed : _successGreen,
              ),
            ),
          ),
        ],
